...
def main():
        ...
        highlights.append(
            {
                ...
                "desc": f"قیمت تقریبی ${pick['priceUsd']} — محصول اصل Liqui Moly",
                "useCaseFa": cat,
                "badge": cat.split()[0],
    ...
    lines.append(f" * Source: liquimolyturkey.com / shop.egemot.com.tr | {data.get('fetchedAt')}")
    ...
    lines.append("      logo,")
    lines.append("      logoAlt: 'لوگوی Liqui Moly — روغن و افزودنی خودرو ساخت آلمان',")
    lines.append(f"      heroImage: LM + 'octane-plus.jpg',")
    lines.append("      heroImageAlt: 'افزودنی بنزین Liqui Moly Octane Plus — ساخت آلمان',")
    lines.append("      heroEyebrow: 'تامین اصل Liqui Moly برای فارسی‌زبانان',")
    ...
    lines.append("      since: 'برند آلمانی · محصول اصل',")
    lines.append("      origin: 'ساخت آلمان',")
    lines.append("      officialUrl: 'https://www.liqui-moly.com/',")
    lines.append("      descriptionFa: 'بیزدوار گروپ تامین‌کننده محصولات اصل Liqui Moly است: روغن موتور، افزودنی بنزین و دیزل، ضدیخ، گیربکس و محصولات مراقبت خودرو. قیمت‌ها به دلار نمایش داده می‌شوند — استعلام و سفارش از واتساپ فارسی.',")
    lines.append("      heroStats: [")
    lines.append(f"        {{ value: '{len(products_sorted)}+', label: 'محصول در کاتالوگ' }},")
    lines.append("        { value: 'USD', label: 'قیمت دلاری' },")
    lines.append("        { value: 'DE', label: 'ساخت آلمان' }")
    lines.append("      ]")
    lines.append("    },")
    lines.append("    trustSignals: [")
    lines.append("      { icon: 'shield', label: 'محصول اصل', desc: 'محصول اصل Liqui Moly با کد محصول و مشخصات فنی' },")
    lines.append("      { icon: 'coin', label: 'قیمت دلاری شفاف', desc: 'قیمت نمایشی دلاری — پیش‌فاکتور قبل از پرداخت' },")
    lines.append("      { icon: 'phone', label: 'مشاوره فارسی', desc: 'انتخاب روغن، ویسکوزیته و افزودنی مناسب خودرو — واتساپ فارسی' },")
    lines.append("      { icon: 'ship', label: 'لجستیک بین‌المللی', desc: 'هماهنگی تامین و حمل تا مقصد پروژه شما' }")
    lines.append("    ],")
    lines.append("    whyBuyFromUs: [")
    lines.append("      { icon: 'droplet', title: 'انتخاب درست روغن و افزودنی', desc: 'اگر نمی‌دانید 5W-30 یا 5W-40، یا Octane Plus مناسب است — مدل خودرو و نوع سوخت را بگویید.' },")
    ...
    lines.append("      { num: '۴', title: 'تامین و حمل', desc: 'تامین محصول اصل و هماهنگی لجستیک بین‌المللی' },")
    ...
    lines.append("      { name: 'تعمیرگاه و سرویس خودرو', desc: 'تامین روغن موتور، افزودنی سوخت و محصولات مراقبت برای سرویس دوره‌ای — سفارش کارتنی یا عمده با پیش‌فاکتور دلاری.', image: LM + 'audiences/workshop.svg', imageAlt: 'تعمیرگاه خودرو — تامین Liqui Moly', icon: 'wrench', models: 'Top Tec · Octane Plus · Cera Tec' },")
    lines.append("      { name: 'ناوگان و تاکسی', desc: 'روغن و افزودنی بنزین/دیزل برای ناوگان تاکسی، پخش و سازمانی — مصرف مداوم، استعلام عمده و زمان‌بندی تامین.', image: LM + 'audiences/fleet.svg', imageAlt: 'ناوگان و تاکسی — تامین Liqui Moly', icon: 'ship', models: '5W-30 · Diesel Additive · Top Tec' },")
    lines.append("      { name: 'موتورسیکلت', desc: 'روغن 4T، محصولات مراقبت و نگهداری موتورسیکلت شهری و آفرود — مناسب کلاچ خیس و گیربکس مشترک.', image: LM + 'audiences/motorcycle.svg', imageAlt: 'موتورسیکلت — Liqui Moly Motorbike', icon: 'bolt', models: '4T Street · Motorbike · 10W-40' },")
    ...
    lines.append("      { q: 'محصول اصل Liqui Moly است؟', a: 'بله — محصول اصل Liqui Moly تامین می‌شود. کد محصول (SKU) روی هر آیتم مشخص است.' },")
    ...
    lines.append("      { q: 'زمان تحویل چقدر است؟', a: 'بسته به موجودی و مقصد، معمولاً چند روز تا چند هفته. زمان دقیق در پیش‌فاکتور اعلام می‌شود.' }")
    lines.append("    ],")
    ...
    lines.append("      { title: 'تامین اصل', desc: 'تامین محصولات اصل Liqui Moly' },")
