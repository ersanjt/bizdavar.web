/**
 * Geo/locale bootstrap — chrome renders immediately; geo locale updates in background
 */
(function () {
  let localeReady = !!window.BIZDAVAR_I18N?.dict;
  let renderSiteChromeOrig = null;
  let geoStripDone = false;

  const IMMEDIATE_FNS = new Set(['renderSiteChrome', 'setupWhatsappLinks', 'renderBreadcrumbs']);
  ...
  function wrapRenderFns() {
    RENDER_FNS.forEach(name => {
      ...
      if (IMMEDIATE_FNS.has(name)) return;
